...
public class InstanceReader {
     ...
     */
    public Instance readInstance() throws ReaderException, TourneeException {
        ...
        if (dureeMax<dureeMin)
            throw new ReaderException("Erreur!","Le temps min est supérieur au temps max");
        instance = new Instance(name,dureeMin,dureeMax,date);
        ...
        while(true) {
            Tournee t = readPaireInLine(scanner);
            if(t == null) {
                break;
            }

            if (!instance.addTournee(t)){
                throw new ReaderException(t.toString(),"ERREUR lors de l'ajout de la tournée");
            }
        }
        return instance;
    }
     ...
     * @throws ReaderException lorsque le format attendu n'est pas respecte
     */
    private Tournee readPaireInLine(Scanner scanner) throws ReaderException, TourneeException {
        String[] values = nextLine(scanner);
        if(values.length == 0) {
            return null;
        }
        ...
        Tournee val;
        try {
            val = new Tournee(new Horaire(values[0]),new Horaire(values[1]));
